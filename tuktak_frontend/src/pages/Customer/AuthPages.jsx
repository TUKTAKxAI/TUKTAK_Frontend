  ...
  if (screen === screens.login) {
    return (
      <section className="auth-screen login-screen">
        ...
        <Field placeholder="비밀번호" type="password" action=" " />
        ...
        <button className="link-row" onClick={() => go(screens.signup)}>회원가입 | 아이디 찾기 | 비밀번호 찾기</button>
      </section>
    )
  }
  ...
  if (screen === screens.signup) {
    return (
      <section className="auth-screen signup-screen">
        ...
        <Field placeholder="비밀번호" type="password" action=" " />
        <Field placeholder="비밀번호 확인" type="password" action=" " />
  ...
  if (screen === screens.terms) {
    return (
      <section className="auth-screen">
        <BackButton onClick={back} />
        <Logo />
        <h2 className="hero-copy huge">고객님의 가입을 위해 약관에 동의해주세요</h2>
          ...
          {signupTerms.map((item, index) => (
            <label className="check-line" key={item}>
              <input
                type="checkbox"
                checked={terms[index]}
                onChange={() => setTerms((current) => current.map((value, valueIndex) => (valueIndex === index ? !value : value)))}
              />
              <span>{item}</span>
            </label>
          ))}
          <div className="button-row sticky-row">
            <PrimaryButton orange onClick={back}>취소</PrimaryButton>
    ...
    return (
      <section className="auth-screen">
        ...
        <div className="phone-row">
          <select defaultValue="KT"><option>KT ▼</option></select>
          <Field placeholder="XXX - XXXX - XXXX" compact />
        </div>
  ...
  if (screen === screens.address) {
    return (
      <section className="auth-screen">
        ...
        <Field placeholder="상세 주소 입력 (ex : 202동 301호)" compact />
